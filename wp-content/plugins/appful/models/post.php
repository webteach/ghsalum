...
class Appful_API_Post {
		...
		if ($appful_api->include_value('content')) {
			...
			foreach ($matches[0] as $match) {
				if (strpos($match, "[poll") !== FALSE) {
					$var = explode("\"", $match);
					$content = str_replace($match, "<iframe class=\"wp-poll\" frameborder=\"0\" scrolling=\"no\" id=\"iframe\" onload=\"resizeIframe(this);\" src=\"". get_option("siteurl", "") ."/wp-content/plugins/appful/plugins/wp-polls.php?id=" . $var[1] . "\"></iframe>", $content);
	...
	function set_thumbnail_value() {
		...
		if(!$this->thumbnail) $this->thumbnail = $attachment->url;
		$this->thumbnail_images = $attachment->images;
		if(count($this->thumbnail_images) == 0 && $this->thumbnail) {
			$this->thumbnail_images = array("thumbnail" => array("url" => $this->thumbnail));
		}
	}
